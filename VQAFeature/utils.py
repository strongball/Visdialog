...
def setDualData(data, lang, device, negsimple = 1):    
    images = data["images"].to(device)
    answers = []
    questions = []
    label = [1]*images.size(0)+[0]*images.size(0)*(negsimple)
    for question in data["questions"]:
        questions.append(question.to(device))
    for answer in data["answers"]:
        answers.append(answer.to(device))
        
    images = images.repeat(1+negsimple,1,1,1)
    questions = questions * (1+negsimple)
    label = torch.FloatTensor(label).to(device)
    
    negs = []
    for _ in range(negsimple):
        i = torch.randint(1, images.size(0), []).int()
        negs = answers[i:] + answers[:i]
    answers  = answers + negs
    return images, questions, answers, label
