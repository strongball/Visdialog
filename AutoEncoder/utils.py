...
def encode(model, device, lang, string):
    string_t = torch.LongTensor(string).unsqueeze(0).to(device)
    feature = model.makeContext(string_t)
    return feature

def decode(model, device, lang, feature):
...
def predit(model, device, lang, question):    
    question_t = torch.LongTensor(question).unsqueeze(0).to(device)
    feature = model.makeContext(question_t)
