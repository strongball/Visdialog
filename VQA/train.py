...
def trainer(args):
    ...
    model = VQAModel(image_setting, question_setting, answer_setting).to(DEVICE)
...
def step(model, criterion, optimizer, **args):
    ...
    sentout, senthidden = model(images_t, questions_t, answers_t["in"])
    
    loss = criterion(sentout.view(-1, sentout.size(2)), answers_t["out"].view(-1))
