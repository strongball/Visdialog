...
def setData(data, lang, device):    
    ...
    answers = trainSentence(data["answers"], lang["<SOS>"], lang["<EOS>"], device)
    answers["in"] = pad_sequence(answers["in"], batch_first=True)
    answers["out"] = pad_sequence(answers["out"], batch_first=True)
